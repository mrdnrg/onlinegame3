...
function assignRandomEquipment() {
    ...
    // Отправляем информацию об экипировке противника
    if (playerIds.length === 2) {
        let player1 = players[playerIds[0]];
        let player2 = players[playerIds[1]];

        io.to(player1.id).emit('opponentEquipment', player2.equipment);
        io.to(player2.id).emit('opponentEquipment', player1.equipment);
    }
}
